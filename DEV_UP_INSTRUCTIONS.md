...
**Repository:** `GlacierEQ/cohere-retrieval-claim-fence`  
**Company lens (independent):** Cohere (`cohere`)  
**Innovation:** Retrieval Claim Fence  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Cohere affiliation, proprietary access, or production deployment.

### Bottleneck
Secure retrieval, multilingual grounding, serving efficiency, and production operations.

### Brick wall
Crossing the prototype-to-production gap without retrieval failure, access leakage, unfair serving, or runaway TCO.

### Mechanism to implement
Fence every claim to supporting passage digests; block uncited factual assertions.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Cohere employment, endorsement, or proprietary systems access.
...
File: `src/retrieval_claim_fence.py`

- Expand `RetrievalClaimFence` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_retrieval_claim_fence.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_retrieval_claim_fence.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Cohere APIs
